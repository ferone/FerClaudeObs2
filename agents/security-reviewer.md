...
- Share critical/high severity findings with `@code-reviewer` via SendMessage so they can account for them in their overall assessment
- If security issues have performance implications (e.g., missing rate limiting under load), flag for `@performance-reviewer`
