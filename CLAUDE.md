...
ferclaudeobs is a portable Claude Code configuration system and Obsidian knowledge management platform. It bundles rules, skills, agents, hooks, settings, and a curated set of 30+ plugins into a single repository you can drop into any project. Clone once, run `/init`, and get a fully configured Claude Code environment on any machine.
...
Or use the skill: `/process-docs`, `/process-docs --mode=extract`, `/process-docs --with-enrich`
...
## Privacy Zones -- Critical

Privacy levels are defined in each domain pack's `pack.json`. Example from the included `philenergy` pack:

| Path | Level | External API |
|------|-------|-------------|
| Confidential folders | confidential | **Never** -- no external API calls |
| Sensitive folders | sensitive | Approval required per session |

**Always** check privacy zones before sending vault content to any external API (Claude API, Gemini, NotebookLM). The `vault-protect.sh` hook enforces this at the tool level.
...
- `/pdf-to-markdown` -- Standalone Claude vision PDF conversion with table preservation
...
- `doc-inspector` -- Compares source documents against vault markdown for quality verification
