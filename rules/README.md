...
### database.md
**Scope**: Path-scoped (`**/migrations/**`, `**/prisma/**`, `**/drizzle/**`, `**/alembic/**`, `**/knex/**`)

Loads when touching migration or ORM files. Migration safety (always reversible, no data loss), query patterns (parameterized, no raw SQL in app code), connection pooling, index awareness.

### error-handling.md
**Scope**: Path-scoped (`src/api/**`, `src/services/**`, `src/middleware/**`, `**/handlers/**`)

Loads when touching backend service code. Error hierarchy, structured error responses, logging levels, retry patterns, circuit breaker awareness, graceful degradation.

### obsidian.md
**Scope**: Path-scoped (`vault/**`, `domain-packs/**`, `config/**`, `scripts/pipeline.py`, `scripts/*.py`)

Loads when touching Obsidian vault files or pipeline scripts. Vault interaction rules, frontmatter requirements, wikilink conventions, privacy zone enforcement, entity registry consistency, pipeline stage ordering.
