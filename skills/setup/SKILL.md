...
Before anything else, delete files inside `.claude/` that exist for the ferclaudeobs repo itself but waste tokens or cause issues at runtime:
- `.claude/README.md` (repo README accidentally copied in)
- `.claude/CONTRIBUTING.md` (repo contributing guide accidentally copied in)
- `.claude/.gitignore` (for the ferclaudeobs repo, not the project — the project has its own .gitignore)
...
**Obsidian indicators**: `scripts/pipeline.py`, `domain-packs/`, `config/active-pack.json`, `vault/`, `.obsidian/`, `inbox/`
...
## Phase 4b: Obsidian Knowledge Management Projects

If Obsidian indicators were detected:

1. Read `config/active-pack.json` to identify the active domain pack
2. Read the pack's `pack.json` for domain name and configuration
3. Add vault-specific sections to project CLAUDE.md:
   - Vault location and inbox location
   - Pipeline commands reference (from the active domain pack)
   - Privacy zones (from pack.json)
   - Active domain pack name and description
4. Ensure `rules/obsidian.md` is enabled
5. Ensure all vault agents are available
6. Ensure `hooks/vault-protect.sh` is registered in settings.json
7. If `config/active-pack.json` doesn't exist, prompt user to run `/setup-vault`
