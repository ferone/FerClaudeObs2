...
Detects hardcoded API keys, tokens, and credentials in file content before writes. Scans for common secret patterns (AWS keys, GitHub tokens, JWT secrets, database connection strings, private keys). Blocks the write and warns the user if a secret pattern is detected.
...
Protects vault privacy zones and sensitive content in Obsidian vault projects. Reads privacy zone configuration from the active domain pack's `pack.json` and blocks writes to files within restricted zones. Ensures sensitive documents (marked as private or confidential) are not accidentally modified or exposed.
